    ...
    <div className="bg-[#121212] border border-[#2c2c2c] rounded-2xl p-6 flex flex-col gap-6 h-full">
      ...
      <div className="flex flex-col gap-3 flex-1">
        {contractTypes.map((type) => (
          ...
          >
            <div>
              <h4 className="text-sm font-black text-white mb-1 group-hover:text-[#EF4444] transition-colors">{type.title}</h4>
              <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">{type.subtitle}</p>
            </div>
            <FiChevronRight className="w-4 h-4 text-gray-600 group-hover:text-[#EF4444] group-hover:translate-x-1 transition-all" />
          </button>
        ))}
      </div>

      {/* Pro Tip */}
      <div className="bg-[#EF4444]/[0.03] border border-[#EF4444]/[0.1] rounded-2xl p-5 space-y-3 mt-auto">
