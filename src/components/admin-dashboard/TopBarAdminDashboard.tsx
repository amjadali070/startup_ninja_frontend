...
interface DashboardTopbarProps {
  ...
  onSettings?: () => void;
  title?: string;
  statusBadges: StatusBadgeInfo[];
}


// A small helper component for rendering the status badges
const StatusBadge: FC<StatusBadgeInfo> = ({ label, value, color }) => {
  const colorClasses = {
    green: {
      bg: 'bg-green-900/50 hover:bg-green-900/80',
      dot: 'bg-green-500',
    },
    yellow: {
      bg: 'bg-yellow-900/50 hover:bg-yellow-900/80',
      dot: 'bg-yellow-500',
    },
    red: {
      bg: 'bg-red-900/50 hover:bg-red-900/80',
      dot: 'bg-red-500',
    },
  };
  ...
  return (
    <div
      className={`flex items-center gap-2 px-4 py-2 rounded-full cursor-pointer transition-colors ${colorClasses[color].bg}`}
    >
      <div className={`w-2 h-2 rounded-full ${colorClasses[color].dot}`} />
      <span className="text-white text-sm font-medium">
        {label}
        {value && <span className="text-gray-300 ml-1">{value}</span>}
      </span>
    </div>
  );
};

const TopBarAdminDashboard: FC<DashboardTopbarProps> = ({
  userName,
  profilePicture,
  email,
  username,
