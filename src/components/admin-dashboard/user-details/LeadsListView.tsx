        ...
        >
          <div className="flex justify-between items-start mb-3">
            ...
            <span className={`px-2 py-1 rounded-md text-[10px] font-medium uppercase ${
              lead.status === 'Closed' ? 'bg-green-500/10 text-green-500' : 'bg-blue-500/10 text-blue-500'
            }`}>
              {lead.status}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-[#242424]">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-400 truncate">{lead.company || 'N/A'}</span>
            </div>
            <div className="flex items-center gap-2">
              <FaDollarSign className="text-gray-500 text-xs" />
              <span className="text-xs text-gray-400">{lead.value ? `$${lead.value}` : '$0'}</span>
            </div>
          </div>
