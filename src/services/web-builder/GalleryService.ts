...
class GalleryService {
     ...
     */
    async uploadMultipleAssets(files: File[]): Promise<UploadMultipleAssetsResponse> {
        try {
            const formData = new FormData();
            files.forEach(file => {
            ...
            const response = await apiClient.post(`${this.baseURL}/upload-multiple`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });
     ...
     */
    async getUserAssets(page: number = 1, limit: number = 50, type?: string): Promise<GetAssetsResponse> {
        try {
    ...
    async deleteAsset(assetId: string): Promise<DeleteAssetResponse> {
        try {
            const response = await apiClient.delete(`${this.baseURL}/assets/${assetId}`);
    ...
    getAssetUrl(userId: string, filename: string): string {
        const baseURL = import.meta.env.VITE_WEB_BUILDER_SERVICE_URL || 'http://localhost:3004';
